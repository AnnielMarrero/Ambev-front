<template>
  ...
  <q-form v-if="!loading" ref="formEdit" @submit="submitEvent">
    ...
    <div :class="centerClassRow">
      <div class="col-6">
        <div class="row q-col-gutter-lg">
          <div class="col-6">
            <q-input v-model="event.producerName" label="Produtor" />
          </div>
          <div class="col-6">
            <q-input v-model="event.producerPhone" label="Tel. Produtor" />
          </div>
        </div>
      </div>
    </div>
    <div :class="centerClassRow">
      <div class="col-6">
        <q-input v-model="event.producerEmail" label="E-mail do produtor" type="email" />
      </div>
    </div>
    ...
    <div :class="centerClassRow">
      <div class="col-6">
        <div class="row">
          ...
          <div class="col-2"><q-toggle v-model="event.madePublicSpace" /></div>
          <div class="col-2 column items-end">
            <q-icon size="xs" name="help" color="negative">
              <q-tooltip anchor="center right" self="top right">
                {{ espacioPublico }}
    ...
    <div :class="centerClassRow">
      <div class="col-6">
        <div class="row">
          ...
          <div class="col-2"><q-toggle v-model="event.madewithMinors" /></div>
          <div class="col-2 column items-end">
            <q-icon size="xs" name="help" color="negative">
              <q-tooltip anchor="center right" self="top right">
                {{ menorIdade }}
    ...
    <div :class="centerClassRow">
      <div class="col-6">
        <div class="row">
          ...
          <div class="col-2"><q-toggle v-model="event.riskActivation" /></div>
          <div class="col-2 column items-end">
            <q-icon size="xs" name="help" color="negative">
              <q-tooltip anchor="center right" self="top right">
                {{ actRisco }}
    ...
    <div :class="centerClassRow">
      <div class="col-6">
        <div class="row">
          ...
          <div class="col-2"><q-toggle v-model="event.brandActivation" /></div>
          <div class="col-2 column items-end">
            <q-icon size="xs" name="help" color="negative">
              <q-tooltip anchor="center right" self="top right">
                {{ marcaEst }}
    ...
    <div :class="centerClassRow">
      <div class="col-6">
        <div class="row">
          ...
          <div class="col-2"><q-toggle v-model="event.universityOpenBar" /></div>
          <div class="col-2 column items-end">
            <q-icon size="xs" name="help" color="negative">
              <q-tooltip anchor="center right" self="top right">
                {{ universitario }}
      ...
      <q-card style="width: 800px; max-width: 80vw">
              ...
              <div v-if="docEvt.category !== '0'" class="col-7 row items-center">
                ...
                <q-file
                  ...
                  :disable="
                    ...
                    userRol.includes(Role.Connections)
                  "
                >
                  <template v-slot:prepend>
        ...
        <q-card-actions v-show="!isFileShowed" align="center">
          ...
          <q-btn no-caps unelevated
            rounded label="Fechar" icon="close" color="primary" v-close-popup />
        </q-card-actions>
      </q-card>
    </q-dialog>
    <q-dialog v-model="showEmailModal" persistent>
      <q-card style="width: 800px; max-width: 80vw">
        <q-card-section>
          <div class="text-h6 text-orange text-center">Upload de e-mail de validação de evento</div>
        </q-card-section>
...
import { ArchiveStatus, Role } from 'src/core/data/enums';
...
const submitEvent = async () => {
  ...
  event.value.civilFireServiceContractId = docEvts.value
    ...
    : null;
  event.value.mobileContractId = docEvts.value ? docEvts.value[3].docId : null;
  ...
  */
  ...
  try {
    await ApiService.put(`/api/Event/Update/${route.params.id}`, event.value);
    //clear data
    ...
    handleError('Falha ao editar evento', err);
  } finally {
...
const uploadFile = async (de: DocEvt) => {
  ...
  if (file && file.name !== '') {
    ...
    formData.append('eventId', route.params.id as string);
...
const uploadEmailFile = async function () {
...
//get current event for edit
const getEvent = async () => {
  loading.value = true;
  try {
    //await delay(1000);
    const resp = await ApiService.get(
      `/api/Event/GetForEdit/${route.params.id}`
    );
    const data = resp.data.result
    event.value = data;
    // load archives for this event
    data.archives.forEach(
      (a: { category: number; id: number; status: number | null }) => {
        if (docEvts.value) {
          const idx =
          ...
          docEvts.value[idx].src = `${process.env.VUE_APP_API_URL}/api/Archive/Show/${a.id}`;
